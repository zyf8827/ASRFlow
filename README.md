...
- **分层流式防退化检查**：
  ...
  - **L2**：非语音累积超时看门狗切段送二遍裁决，配合 Consistency Guard 校验防幻觉（详见 [流式输出退化防护设计](docs/design_degenerate_rows_guard_2026-09-18.md)）。
- **架构完全解耦**：二遍模型通过 HTTP 接口解耦调用独立 vLLM 进程，网关依赖轻量（vLLM 不进服务依赖）。
...
- **开箱即用的观测探针**：内置 HTTP `/healthz`、`/ready` 健康探针与 Prometheus `/metrics` 指标。
...
依赖准备：Python ≥ 3.10。无需下载模型权重即可体验完整 WebSocket 推流与识别链路：
...
运行单元测试（100+ 测试项，全 Mock 引擎，无需 GPU）：
...
网关与 vLLM 独立容器编排、多架构（CPU / NVIDIA GPU / 华为昇腾 NPU）支持及单/多实例配置见 [`deployment/`](deployment/) 目录：
...
asrflow/
├── core/             # 核心组件实现
│   ├── streaming_asr/ # 首遍流式引擎（ONNX 多路合批 / Mock）
│   ├── final_asr/     # 二遍异步引擎（vLLM HTTP / OpenAI / Mock）与微批队列
│   ├── vad/           # VAD 引擎（FSMN-VAD / Mock）
...
├── fixtures/          # 公开可下载的测试音频（sample_16k.wav）
├── scripts/           # 模型下载、ONNX 导出与维护脚本
├── tests/             # 单元测试与 E2E 测试套件（全部基于 Mock 引擎）
...
- [流式输出退化分层检查设计 (docs/design_degenerate_rows_guard_2026-09-18.md)](docs/design_degenerate_rows_guard_2026-09-18.md)
